...
#### Model Types
Since this is a classification problem, the models I will be creating will be: null, baseline, logistic, elastic net, nearest neighbors, random forest, and boosted tree. I have already created the first three models and will eventually tune the others. The null and the baseline models are currently using the basic recipe and the logistic model is currently using the main recipe for non-tree models, to make sure that recipe is in fact working. For the final project, I believe every model needs to be fit on both the basic recipe and the main recipe, which I will do. Each model type will have its own R script that will include the model specifications, workflows, and fits for all models under that type category.

## Model Analysis
```{r}
#| label: tbl-metrics
#| fig.cap: Assessment metrics for null, baseline, and logistic models
#| message: false
#| echo: false
